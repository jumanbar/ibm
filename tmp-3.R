...
M  <- 10 ^ ex
tf <- function(i) round(2000 + (2000 / 2.4) * ex[i])

for (i in 1:length(M)) {
  cat('Va por el', i, 'de', length(M), '\n')
  run <- ibm(landsLmax_=0, levelSeeds=0, N_0=1, levelFocus=0,
             mpd0=6, sizeMode="random", grassMode='bholt',
             trs0=0.1, yield=2000 * 81, tfinal=tf(i),
             M=M[i], saveRecord=FALSE, verboso=TRUE)
  nombre <- paste('corrida-M=', round(M[i], 2), '-', Sys.Date(),
                  '-mandinga-onepatch-2.RData', sep='')
